...
class Map:
	def __init__(self):
		...
		self.definitions = {} # dictionary of map definitions
...
def DMPREAD(dmp):
	...
	for index, line in enumerate(dmp):
		...
		for x in split(match.group(2), ','):
			...
			map.definitions[match.group(1)].append(instance)

	# grab tiles
	start_index = -1
	current_z = -1
	def_len = len(map.definitions.keys()[0])
	y_list = []
...
def DMPWRITE(map, dmp):
	...
	definitions = map.definitions.keys()
	definitions.sort(dosort)

	for x in definitions:
		dmp.write('"%s" = (' % x)
		for index, o in enumerate(map.definitions[x]):
			if index:
				dmp.write(',')
	...
	for z in xrange(len(map.tiles)):
		dmp.write('(1,1,%i) = {"\n' % (z + 1))
		for y in xrange(len(map.tiles[z])-1, -1, -1):
			for x in map.tiles[z][y]:
				dmp.write(x)
			dmp.write('\n')
		dmp.write('"}\n\n')
...
if __name__ == '__main__':
	map = DMPREAD('test.dmp')
	DMPWRITE(map, 'testing.dmp')