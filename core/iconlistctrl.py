...
class IconListCtrl(wx.ScrolledWindow):
    ...
    def OnPaint(self, event):
        temp_buffer = wx.EmptyBitmap(*self.GetVirtualSize())
        dc = wx.BufferedPaintDC(self, temp_buffer, wx.BUFFER_VIRTUAL_AREA)
    ...
    # Note: this function might not be needed at all?
    def GetNextSelected(self, index):
        # TODO: ULC calls GetNextItem, and is able to do more complicated things
        # At the moment, just need simple default case
        maxSel = self.GetSelectedItemCount()

        if maxSel <= 1:
            return wx.NOT_FOUND

        # FIXME: Subclass the selectionstore to add this kind of functionality

        idx = 0

        for i in xrange(0, maxSel-1):
            if self._selStore._itemsSel[i] == index:
                idx = i + 1

        if idx > maxSel:
            return self._selStore._itemsSel[0]

        return self._selStore._itemsSel[idx]
